...
export class SurveyComponent implements OnInit {
  ...
  answerFieldClick(event: any){
    ...
    if (event.target.id.split('-')[0] === 'choice'){
      id = event.target.id.split('-')[2];
      console.log(id);
    }
    const elms = document.getElementsByClassName('scaItem');
    for (let i = 0; i < elms.length; i++){
      this.render.removeClass(elms[i], 'scaCurrent');
  ...
  scButtonClick(event: any){
    let surveryContainer;
    const id = event.target.id.split('-')[1];
    if (event.target.type === 'number'){
      surveryContainer = event.target.parentNode.parentNode.parentNode;
    } else {
      surveryContainer = event.target.parentNode;
    }
    ...
    console.log('SCCLICK: ' + currentContainerNo);
