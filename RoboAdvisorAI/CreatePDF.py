...
def create_html(data, names):
    ...
    for idx, x in enumerate(data):
        ...
        sepetlerHtml += "<div style='display: inline-block ;width:60%; text-align:center; margin:auto;padding: 30px 0 30px 0'>"
        sepetlerHtml += "<h5 class='categoryTitle'>"+names[idx]+"</h5>"
        sepetlerHtml += "<table>"
        subArray = []
        for aidx, a in enumerate(x[0][0]):
            sepetlerHtml += "<tr>"
            sepetlerHtml += "<td class='dotrow'><span class='dot' id='dot"+str(idx)+""+str(aidx)+"'></span></td>"
            sepetlerHtml += "<td>"
            sepetlerHtml += str(a)
            sepetlerHtml += "</td>"
            sepetlerHtml += "<td style='width:80px'>% "
            sepetlerHtml += str(x[0][1][aidx])
            sepetlerHtml += "</td>"
            sepetlerHtml += "</tr>"
            subArray.append(x[0][1][aidx])
...
#datax = [[]]
#datax1 = []
#basknames = ["BASKET NAME1","BASKET NAME2","BASKET NAME3","BASKET NAME4","BASKET NAME5","BASKET NAME6","BASKET NAME7","BASKET NAME8","BASKET NAME","BASKET NAME","şkşçk"]
#datanames = ["werasdasdqwdasd","zxcasdasdasdasd","ertasdwdasdwqdasd"]
#datavalues = ["12.3","23.4","16.4"]
#basknames.reverse()
